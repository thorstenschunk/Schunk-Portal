...
export async function GET(req:NextRequest){
  try{
    ...
    let itemQuery=db.from('project_items').select('id,item_type,title,priority,status,construction_site_id,created_at,construction_sites(title,project_no)').in('item_type',['defect','problem','message']).neq('status','Erledigt').neq('status','Geklärt').order('created_at',{ascending:false}).limit(12);if(!isWide&&siteIds.length)itemQuery=itemQuery.in('construction_site_id',siteIds);const {data:items,error:itemErr}=await itemQuery;if(itemErr)throw itemErr;const urgent=(items||[]).filter((x:any)=>x.priority==='Dringend'&&['defect','problem'].includes(x.item_type));const messages=(items||[]).filter((x:any)=>x.item_type==='message');return NextResponse.json({sites,urgent,messages,stats:{sites:siteCount,todayMinutes:(times||[]).reduce((s:any,x:any)=>s+(x.total_minutes||0),0),openReports,openTasks:openTasks||0,urgent:urgent.length,messages:messages.length}});
