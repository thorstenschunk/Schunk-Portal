...
export async function POST(req:NextRequest){try{
 ...
 const db=supabaseAdmin();const broad=u.roles.some((r:string)=>['admin','office','foreman'].includes(r));const uploadVisibility=broad?(b.visibility||'site_members'):'site_members';const uploadVisibleTo=broad&&uploadVisibility==='selected'&&Array.isArray(b.visible_to)?b.visible_to:[];
 const rawExt=String(b.file_name).includes('.')?(String(b.file_name).split('.').pop()||''):'';const ext=rawExt?'.'+rawExt.replace(/[^a-z0-9]/gi,'').slice(0,8):'';
 ...
 const {data:file,error:fe}=await db.from('files').insert({entity_type:b.entity_type,entity_id:b.entity_id,category:b.category||'attachment',title:b.title||b.file_name,file_name:b.file_name,storage_path:path,mime_type:b.mime_type||null,size_bytes:size,description:b.description||null,section_id:b.section_id||null,report_day_id:b.report_day_id||null,visibility:uploadVisibility,visible_to:uploadVisibleTo,upload_status:'pending',uploaded_by:u.id}).select().single();if(fe)throw fe;
