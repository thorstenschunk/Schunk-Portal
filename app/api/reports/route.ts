...
try{const {data:day,error:dayErr}=await db.from('work_report_days').insert({report_id:report.id,work_date:b.work_date,work_description:String(b.work_description).trim(),remarks:b.remarks||null,created_by:u.id}).select().single();if(dayErr)throw dayErr;const {error:me}=await db.from('work_report_members').insert(members.map((m:any)=>({report_id:report.id,report_day_id:day.id,user_id:m.user_id,start_time:m.start_time,end_time:m.end_time,pause_minutes:m.pause_minutes,travel_setup_minutes:m.travel_setup_minutes,total_minutes:m.total_minutes})));if(me)throw me;if(Array.isArray(b.materials)&&b.materials.length){const mats=b.materials.filter((x:any)=>String(x.description||'').trim()).map((x:any)=>({report_id:report.id,report_day_id:day.id,section_id:b.section_id||null,quantity:x.quantity?Number(x.quantity):null,unit:x.unit||null,description:String(x.description).trim()}));if(mats.length){const {error:merr}=await db.from('work_report_materials').insert(mats);if(merr)throw merr;}}}catch(child){await db.from('work_reports').delete().eq('id',report.id);throw child;}
...
if(b.action==='continue'){
  if(old.locked_at)throw new ApiError(409,'Dieser Rapport ist bereits endgültig abgeschlossen.');
  const workDate=String(b.work_date||'');if(!workDate)throw new ApiError(400,'Datum fehlt.');
  const today=new Intl.DateTimeFormat('en-CA',{timeZone:'Europe/Berlin',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date());
  if(!canSeeAll(u)&&workDate!==today)throw new ApiError(400,'Mitarbeiter dürfen nur den aktuellen Arbeitstag ergänzen.');
  const {data:exists}=await db.from('work_report_days').select('id').eq('report_id',b.id).eq('work_date',workDate).maybeSingle();if(exists)throw new ApiError(409,'Für diesen Tag existiert bereits ein Tagesabschnitt.');
  if(!Array.isArray(b.members)||!b.members.length)throw new ApiError(400,'Mindestens eine Rapportzeit ist erforderlich.');
  let dayMembers:any[];
  if(!canSeeAll(u)){
  ...
  const {error:me}=await db.from('work_report_members').insert(dayMembers.map((m:any)=>({report_id:b.id,report_day_id:day.id,user_id:m.user_id,start_time:m.start_time,end_time:m.end_time,pause_minutes:m.pause_minutes,travel_setup_minutes:m.travel_setup_minutes,total_minutes:m.total_minutes})));if(me)throw me;
  const mats=(Array.isArray(b.materials)?b.materials:[]).filter((x:any)=>String(x.description||'').trim()).map((x:any)=>({report_id:b.id,report_day_id:day.id,section_id:old.section_id||null,quantity:x.quantity?Number(x.quantity):null,unit:x.unit||null,description:String(x.description).trim()}));if(mats.length){const {error:merr}=await db.from('work_report_materials').insert(mats);if(merr)throw merr}
  await db.from('work_reports').update({work_date:workDate,updated_at:new Date().toISOString()}).eq('id',b.id);await audit(u.id,'continue','work_report',b.id,{work_date:workDate,report_day_id:day.id});return NextResponse.json(day);
}
