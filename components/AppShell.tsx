...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="app-shell">
    ...
    <div className="content-shell"><header className="topbar"><div className="topbar-brand"><img className="mobile-top-logo" src="/logo.png" alt="Tischlerei Schunk"/><div className="topbar-copy"><span className="eyebrow">DESIGN TISCHLEREI SCHUNK</span><strong>SCHUNK PORTAL</strong></div></div>{auth.has('admin.users.manage')?<Link href="/einstellungen" className="icon-btn" title="Einstellungen" aria-label="Einstellungen"><Settings size={22}/></Link>:<span/>}</header><main className="page">{children}</main></div>
  </div>;
}
