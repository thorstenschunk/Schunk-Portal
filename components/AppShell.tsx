...
export function AppShell({ children }: { children: React.ReactNode }) {
  const auth = useAuth(); const pathname = usePathname();const [unseen,setUnseen]=useState<any>({});const [mobileMenuOpen,setMobileMenuOpen]=useState(false);
  useEffect(()=>{if(auth.accessToken)apiFetch<any>('/api/unseen',auth.accessToken).then(setUnseen).catch(()=>setUnseen({}))},[auth.accessToken,pathname]);useEffect(()=>{setMobileMenuOpen(false)},[pathname]);
  ...
  return <div className="app-shell">
    <aside className={mobileMenuOpen?"sidebar mobile-open":"sidebar"}>
      ...
      <nav>{nav.filter(n=>auth.has(n.permission)).map(n=>{const I=n.icon;const active=n.href==='/'?pathname==='/':pathname.startsWith(n.href);return <Link key={n.href} href={n.href} className={active?'nav-link active':'nav-link'}><I size={20}/><span>{n.label}</span>{(()=>{const key=n.href==='/rapporte'?'reports':n.href==='/bestellungen'?'orders':n.href==='/nachrichten'?'messages':n.href==='/maengel'?'defects':n.href==='/aufgaben'?'tasks':null;return key&&unseen[key]>0?<b className="nav-count">{unseen[key]}</b>:null})()}</Link>})}</nav>
