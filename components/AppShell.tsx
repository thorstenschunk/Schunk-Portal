...
export function AppShell({ children }: { children: React.ReactNode }) {
  const auth = useAuth(); const pathname = usePathname();const [messageCount,setMessageCount]=useState(0);const [orderCount,setOrderCount]=useState(0);const [mobileMenuOpen,setMobileMenuOpen]=useState(false);
  useEffect(()=>{if(auth.accessToken)apiFetch<any[]>('/api/internal-messages',auth.accessToken).then(x=>setMessageCount(x.filter((m:any)=>m.status!=='Geklärt'&&m.last_sender_id!==auth.me?.id&&!m.read_at).length)).catch(()=>setMessageCount(0))},[auth.accessToken,pathname,auth.me?.id]);useEffect(()=>{if(auth.accessToken)apiFetch<{count:number}>('/api/purchase-requests/unseen',auth.accessToken).then(x=>setOrderCount(x.count||0)).catch(()=>setOrderCount(0))},[auth.accessToken,pathname]);useEffect(()=>{setMobileMenuOpen(false)},[pathname]);
  ...
  return <div className="app-shell">
    <aside className={mobileMenuOpen?"sidebar mobile-open":"sidebar"}>
      ...
      <nav>{nav.filter(n=>auth.has(n.permission)).map(n=>{const I=n.icon;const active=n.href==='/'?pathname==='/':pathname.startsWith(n.href);return <Link key={n.href} href={n.href} className={active?'nav-link active':'nav-link'}><I size={20}/><span>{n.label}</span>{n.href==='/nachrichten'&&messageCount>0?<b className="nav-count">{messageCount}</b>:null}{n.href==='/bestellungen'&&orderCount>0?<b className="nav-count">{orderCount}</b>:null}</Link>})}</nav>
